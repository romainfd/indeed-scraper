...
const FAST = false;
// Between pages of offers
const DELAY = !FAST * 60; // in seconds
// Total duration for company collection and infos collection
const THROTTLING = !FAST * 10 * 60 * 60; // in seconds
...
module.exports.getCompanies = async function(jobs) {
  const companiesUrlsList = await Promise.all(jobs.map(collectCompanyUrl));
  let companiesUrls = new Set(companiesUrlsList); // remove duplicates
  companiesUrls.delete(null); // remove potential nulls
  companiesUrls = [...companiesUrls];
  console.log("Collecting companies infos with URLs");
  return {
    companiesUrls: companiesUrlsList,
    companies: Object.assign(
      {},
      ...(await Promise.all(companiesUrls.map(getCompanyInfos)))
    )
  };
};
...
Query.prototype.getJobs = function() {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    async function getSomeJobs(self, jobs) {
      // random throttling
      await new Promise(r => setTimeout(r, DELAY * 1000 * Math.random()));

      console.info("Getting some jobs - " + self.query);
      request(self.url(), async (error, response, body) => {
...
function parseJobList(body, host, excludeSponsored) {
  ...
  if ($("title").text() === "hCaptcha solve page") {
    return { error: "Error: Hitting hCaptcha page" };
  }
  ...
  const jobObjects = filtered
    .map((i, e) => {
      ...
      const url = "https://" + host + job.find(".jobTitle > a").attr("href");
...
function collectCompanyUrl(job) {
  return new Promise(async resolve => {
    if (!job.url) {
      resolve(null);
    }
    // random throttling
    await new Promise(r => setTimeout(r, THROTTLING * 1000 * Math.random()));

    request(job.url, (error, response, body) => {
      console.log(job.url);
      const $ = cheerio.load(body);
      const companyUrl = $(".jobsearch-JobInfoHeader-subtitle a").attr("href");
      if (companyUrl) {
        resolve(companyUrl.split("?")[0]);
      } else {
        console.warn("Couldn't find company URL for " + job.url);
        resolve(null);
      }
    });
  });
...
function getCompanyInfos(companyUrl) {
  return new Promise(async resolve => {
    // random throttling
    await new Promise(r => setTimeout(r, THROTTLING * 1000 * Math.random()));

    request(companyUrl + "/about", (error, response, body) => {
      console.log(companyUrl);
      const $ = cheerio.load(body);
      const infosList = $("span[data-testid]")
        .map((_, span) => ({
          [$(span).attr("data-testid")]: $(span).text()
        }))
        .get();
      let companyDetails = {};
      if (infosList.length > 0) {
        companyDetails = Object.assign({}, ...infosList);
      }
      resolve({ [companyUrl]: companyDetails });
    });
  });
}
