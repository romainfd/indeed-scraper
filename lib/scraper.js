...
// For company collection and infos collection
const THROTTLING = 3 * 60 * 60; // in seconds
...
module.exports.getCompanies = async function(jobs) {
  let companiesUrls = await Promise.all(jobs.map(collectCompanyUrl));
  companiesUrls = new Set(companiesUrls); // remove duplicates
